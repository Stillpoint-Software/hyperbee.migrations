# Hyperbee.Migrations.Providers.Couchbase.SquashCli

Couchbase `ISquashCliProvider` implementation for the `hyperbee-migrations`
CLI. Reference this package from a migration project to enable
`hyperbee-migrations squash --provider couchbase` codegen against your
migration assembly.

This package is independent from the main
`Hyperbee.Migrations.Providers.Couchbase` package so that production
...
```bash
dotnet add package Hyperbee.Migrations.Providers.Couchbase.SquashCli
```
