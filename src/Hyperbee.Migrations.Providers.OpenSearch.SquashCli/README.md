# Hyperbee.Migrations.Providers.OpenSearch.SquashCli

OpenSearch `ISquashCliProvider` implementation for the `hyperbee-migrations`
CLI. Reference this package from a migration project to enable
`hyperbee-migrations squash --provider opensearch` codegen against your
migration assembly.

This package is independent from the main
`Hyperbee.Migrations.Providers.OpenSearch` package so that production
...
```bash
dotnet add package Hyperbee.Migrations.Providers.OpenSearch.SquashCli
```
