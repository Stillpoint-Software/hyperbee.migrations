...
### Phase 2: OpenSearch squash codegen (R-P4, R-P5-R-P9) (~2 weeks) -- moved from Phase 4 (2026-05-11) ☑ COMPLETE 2026-05-11
...
- **2.8 Unit tests per component (R-P8)** ☑ COMPLETE 2026-05-11 (shipped with Tasks 2.1-2.7) -- 186 new unit tests across 8 test classes; full squash suite 264 -> 450 on net10.
- **2.9 Determinism gate integration test (R-P5)** ☑ COMPLETE 2026-05-11 -- [OpenSearchSquashDeterminismTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/OpenSearchSquashDeterminismTests.cs) guarded by `#if INTEGRATIONS`. 3 integration tests: empty-cluster determinism, populated-cluster determinism (creates 2 prefixed indices, captures twice via the full strategy pipeline, asserts byte-equal), index-creation-order independence (re-creates indices in reverse order, asserts byte-equal). Uses the shared `OpenSearchTestContainer` (no per-test container spin-up); cleanup via TestCleanup drops all `detgate_*` prefixed objects.
- **2.10 Verification-round integration test (R-P6)** ☑ COMPLETE 2026-05-11 -- [OpenSearchSquashVerificationTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/OpenSearchSquashVerificationTests.cs) guarded by `#if INTEGRATIONS`. 2 integration tests: empty-cluster round-trip (Success baseline), populated-cluster round-trip (full A4 byte-equality round with apply-and-recapture). Apply helper walks the canonical JSON sections and PUTs each `verifyround_*` prefixed item back via the corresponding REST endpoint (`/_component_template/<name>`, `/_index_template/<name>`, `/<index>` with settings+mappings+aliases). The full apply path covering all sections (alias graph, ingest pipelines, ISM policies) ships in the Phase 5 CLI; this test exercises the load-bearing index_template + component_template + index_metadata sections sufficient to prove the contract. Defense-in-depth assertions confirm the generated content contains the structural objects we created (not testing an empty-pass-through).
- **2.11 CLI integration test (R-P9)** -- DEFERRED to Phase 5 (consistent with Aerospike Phase 1 Task 1.11). The `dotnet hyperbee-migrations squash --provider opensearch` CLI verb depends on the cross-provider System.CommandLine infrastructure (v1 plan deferred Tasks 7.1-7.7); ships once for all four providers in Phase 5 release prep. Underlying machinery already proves the same C12 byte-stability via Task 2.9 integration tests.
- **2.12 ADR-0019 amendment check (R-P7)** ☑ COMPLETE 2026-05-11 -- NO CHANGES REQUIRED. Despite being the hardest provider (multi-axis plugin matrix, JSON-bodied canonical form, opaque painless content), all 5 contract interfaces absorbed OpenSearch cleanly. The validation hypothesis from the order-revision decision (Aerospike -> OpenSearch -> MongoDB -> Couchbase) is confirmed: if OpenSearch fits, MongoDB and Couchbase will fit. Appendix B contract snapshot remains unchanged.
- **2.13 Phase 2 boundary** ☑ COMPLETE 2026-05-11 -- ☑ unit suite green (squash 450/450 on net10, +186 new); ☑ core suite green (356/356); ☑ integration project builds clean both with `-p:EnableIntegrationTests=true` (0 errors) and without; ☑ plan checkboxes flipped; ☑ pre-existing parser bug found + fixed along the way. Ready for Phase 3 MongoDB.
...
### Phase 3: MongoDB squash codegen (R-P2, R-P5-R-P9) (~1.5 weeks) -- moved from Phase 2 (2026-05-11) ☑ COMPLETE 2026-05-11
...
- **3.1 `MongoDBTopologySignature`** ☑ COMPLETE 2026-05-11 -- ~280 LOC; captures 8 axes: server major.minor, FCV, deployment topology (`Standalone`/`ReplicaSet`/`Sharded` -- with `Sharded` taking precedence over ReplicaSet for mongos responses), replica set name (diagnostic only; not compared), database name, default read concern, default write concern, storage engine. Probes via admin commands: `buildInfo` (required; uses `versionArray` with fallback to version string), `hello` (required; deployment-topology inference), `getParameter:featureCompatibilityVersion` + `serverStatus` (optional, graceful-degradation when caller lacks privileges -- recorded as empty rather than failing). Read/write concerns read from `IMongoClient.Settings` with no privilege requirement. **32 unit tests cover:** SchemaVersion + ProviderId stability, IsCompatibleWith across all axes (major, FCV, deployment, database, storage, read/write concerns), minor-version tolerated, replica-set-name tolerated (the "diagnostic-only axis" pattern), cross-provider rejection (vs OpenSearch), case-insensitive deployment compare; plus internal parsers (versionArray + version-string fallback, deployment classification with sharded-takes-precedence rule, FCV extraction, storageEngine extraction, null/missing-field paths). **Cross-provider observation:** FCV played the Edition-axis role exactly as predicted -- single scalar, fits cleanly in the existing Properties bag. Simpler than OpenSearch's plugin matrix; same contract shape absorbs both. **MongoDB-specific lesson:** the sharded-deployment edge case (mongos response includes BOTH `msg=isdbgrid` AND `setName` for the config replica set) is handled by checking `msg` first -- watching for similar precedence-order edge cases in Couchbase Phase 4 deployment classification.
- **3.2 `MongoDBDataOpClassifier`** ☑ COMPLETE 2026-05-11 -- ~190 LOC; dual-shape classifier mirroring Aerospike + OpenSearch. Statement form (per `MongoStatementParser`): `INSERT INTO` -> data op; `CREATE COLLECTION`, `DROP COLLECTION`, `CREATE [UNIQUE] INDEX`, `DROP INDEX` -> structural. Call-site form: `.InsertOne*` / `.InsertMany*` / `.UpdateOne*` / `.UpdateMany*` / `.ReplaceOne*` / `.DeleteOne*` / `.DeleteMany*` / `.FindOneAndUpdate*` / `.FindOneAndReplace*` / `.FindOneAndDelete*` / `.BulkWrite*` -> data op; `.Find*` / `.CountDocuments*` / `.EstimatedDocumentCount*` / `.Distinct*` / `.Aggregate*` / `.Watch*` -> read; `.CreateCollection*` / `.DropCollection*` / `.RenameCollection*` / `.CreateView*` / `.ListCollections*` / `.ListDatabases*` / `.DropDatabase*` / `.RunCommand*` / sub-client paths `.Indexes.<m>(`, `.Settings.<m>(` -> structural. **Receiver-anchoring trade-off decision:** unlike Aerospike (`_client.*`) and OpenSearch (`_client.*`), MongoDB code routes through local `collection` / `db` variable names, so the classifier anchors only on the leading `.` plus the MongoDB-distinctive method name -- no receiver-name filter. False positives possible (a user class with its own `InsertOne` method) but safer than false negatives under default-deny; operators annotate with `[StructuralOnly]` to suppress. Documented in the classifier docstring. **MethodCallTail pattern** (Phase 2 carry-forward) supports generic method invocations (`InsertOne<TDoc>(doc)`). Non-determinism scan identical catalog to Aerospike/OpenSearch. **37 unit tests cover** statement-form (6) + call-site data ops (8) + reads (5) + structural sub-clients (7) + default-deny (2) + non-determinism (7) + ScanNonDeterminism helper (2). 37/37 on first run -- contract pattern + carry-forward lessons paid off.
- **3.3 `MongoDBStatementClassifier`** ☑ COMPLETE 2026-05-11 -- ~80 LOC; thin projection over the existing `MongoStatementParser`. 6 kinds map 1:1 from `MongoStatementType`: `CreateCollection`, `DropCollection`, `CreateIndex`, `CreateUniqueIndex`, `DropIndex`, `Insert`. Output record: `ClassifiedStatement(Kind, DatabaseName, CollectionName, ObjectName, Body, Detail)` -- mirrors Aerospike + OpenSearch shape with an additional `DatabaseName` slot (MongoDB's `db.collection` reference shape). Default-deny on parser failure with diagnostic in Detail. **13 unit tests cover** each kind (CREATE COLLECTION, DROP COLLECTION, CREATE INDEX, CREATE UNIQUE INDEX, DROP INDEX, INSERT INTO) + compound-field index + backtick-quoted identifiers + unknown verb + syntax error + empty / null / whitespace fallback paths. 13/13 first run -- the Aerospike + OpenSearch precedent shape absorbed MongoDB's grammar cleanly. Identical pattern across THREE thin-projection classifiers now (Aerospike + OpenSearch + MongoDB); Couchbase Phase 4 will be the fourth.
- **3.4 `MongoDBSnapshotCanonicalizer`** ☑ COMPLETE 2026-05-11 -- ~190 LOC; section-headered JSON form per the OpenSearch Phase 2 precedent (NOT statement form -- MongoDB structural state includes validators, time-series options, view pipelines that are too rich for the narrow MongoStatementParser grammar). Recognized sections: `[collections]` / `[indexes]` / `[validators]` / `[views]`; unknown section names pass-through canonicalized. **Ephemeral catalog** (matched at every nesting level): `uuid` (server-generated collection identifier), `readOnly` (runtime flag), `v` (index version 1/2; server-managed and version-dependent -- stripping makes canonical output cross-version stable; applies to both `idIndex.v` and per-index `v`), `ns` (legacy server-injected namespace; deprecated in 4.4+ but still emitted by some hosted offerings). **Opaque-content preservation** matching OpenSearch precedent: aggregation pipeline stages in view definitions, JSON schema validators, partialFilterExpression queries on indexes all ride through as JSON value content; canonicalizer never parses BSON semantically. Cross-platform LF normalization (Phase 2 carry-forward). Array order preserved (pipeline stages depend on declared order). Numbers preserved via `GetRawText` (BSON numeric encoding can produce string-as-int / float / scientific forms; preserve operator representation). **26 unit tests cover** section parsing + ordering + case-insensitive headers; recursive key sort; array order preservation; ephemeral stripping (uuid, readOnly, v at index AND idIndex levels, legacy ns); opaque BSON content preservation (aggregation pipelines, partialFilterExpression); idempotence (C12 baseline); divergent UUIDs / divergent index v values producing byte-equal output (cross-MongoDB-version stability proof); CRLF normalization; number-representation preservation; invalid-JSON error handling; null-input guard; EmitScript = Canonicalize identity; Ephemeral catalog completeness. 26/26 first run -- OpenSearch precedent + ephemeral-strip pattern absorbed MongoDB without surprise.

**Plan rationale revision:** the original plan called for statement-form emission (CREATE COLLECTION + CREATE [UNIQUE] INDEX statements). The actual implementation ships JSON-section form because MongoDB's structural state (JSON schema validators, time-series options, view pipelines, capped-collection settings, partialFilterExpression on indexes) is too rich for the existing MongoStatementParser grammar. Statement-form would lose state; JSON-section form is lossless and matches the OpenSearch precedent established by the Task 2.0 painless spike (opaque content + structural canonical split). The CLI's apply path (Phase 5) handles the JSON-section form natively.
- **3.5 `IntrospectionSnapshotStrategy : ISquashStrategy`** ☑ COMPLETE 2026-05-11 -- ~430 LOC across 3 files. Context + capture helper + strategy + initial DI wiring. [MongoDBSquashGenerationContext.cs](../../../src/Hyperbee.Migrations.Providers.MongoDB/Squash/MongoDBSquashGenerationContext.cs): `IMongoClient` + `DatabaseName` + delegate-injected capture, mirrors Aerospike/OpenSearch shape. [MongoDBSnapshotCapture.cs](../../../src/Hyperbee.Migrations.Providers.MongoDB/Squash/MongoDBSnapshotCapture.cs): production helper that probes `listCollections` + per-collection `Indexes.ListAsync`. Output uses `CanonicalExtendedJson` (per Extended JSON v2 spec) so BSON-specific types (ObjectId, BinData, Decimal128, BsonDateTime) round-trip losslessly. Filters: skip `system.*` collections (operational infrastructure), skip implicit `_id_` indexes (automatic on every collection, adds diff noise). [IntrospectionSnapshotStrategy.cs](../../../src/Hyperbee.Migrations.Providers.MongoDB/Squash/IntrospectionSnapshotStrategy.cs): orchestrator -- same shape as Aerospike Task 1.5 + OpenSearch Task 2.5; optional `ILogger<IntrospectionSnapshotStrategy>` injection (Sev 2 G carry-forward); `MigrationSourceRoot { get; init; }` for the Roslyn source-scanner gate (filled in a follow-up); guard rails return `Failed` with helpful detail. Returns `ContentKind.CanonicalJson`. **DI wiring**: `AddMongoDBMigrations` now registers canonicalizer + classifier + strategy as singletons; the full `SquashStrategyDescriptor` composition lands in Task 3.6. **16 unit tests cover** ComposeBlob shape + sections-in-order + empty/null inputs + canonicalizer round-trip; CaptureAsync guard rails (null client, empty database name); strategy guard rails (null/empty/wrong context); constructor validation; provider-id wiring; null-logger acceptance.
- **3.6 `MongoDBSquashVerifier`** ☑ COMPLETE 2026-05-11 -- ~150 LOC; A4 byte-equality round. Structurally near-copy-paste from Aerospike Task 1.6 + OpenSearch Task 2.6 -- same ctor (canonicalizer), same `CaptureFromGeneratedAsync` init-property, same `VerifyAsync` flow, same SummarizeDiff truncation. **Full DI wiring complete this task**: `AddMongoDBMigrations` registers `MongoDBSquashVerifier` and the composed `SquashStrategyDescriptor` whose ctor `EnsureValid` asserts ProviderId agreement. **19 unit tests cover** verifier (12: provider-id wiring, constructor null-guard, guard rails, Success path, divergent-snapshot Failed with diff naming both index names, ephemeral-only-difference Success defense-in-depth -- proves uuid + v stripping makes A/B byte-equal, exception cause capture, cancellation, SummarizeDiff truncation) + DI wiring (7: each component resolves, descriptor passes EnsureValid, singleton semantics, descriptor components are the same instances as direct resolutions). 19/19 first run. **Cross-provider observation:** verifier shape is now byte-for-byte structurally identical across THREE providers (Aerospike, OpenSearch, MongoDB). The `ISquashVerifier` contract is rock-solid -- Couchbase Phase 4's verifier will be a fourth identical instance.
- **3.7 MongoDBMigrationSourceScanner** ☑ COMPLETE 2026-05-11 -- Roslyn-based source scanner (~190 LOC), mirrors Aerospike + OpenSearch scanner shape with MongoDB-specific verb set: `InsertOne(Async)`, `InsertMany(Async)`, `UpdateOne(Async)`, `UpdateMany(Async)`, `ReplaceOne(Async)`, `DeleteOne(Async)`, `DeleteMany(Async)`, `FindOneAndUpdate(Async)`, `FindOneAndReplace(Async)`, `FindOneAndDelete(Async)`, `BulkWrite(Async)`. Non-determinism scan identical to the other 3 providers' scanners. **Receiver-anchoring trade-off** documented: unlike Aerospike + OpenSearch (whose scanners anchor on `_client.*` receivers), MongoDB code routes through local `collection`/`db` variables. The scanner matches method names without receiver-name filtering. False positives possible; operators annotate with `[StructuralOnly]` to suppress. Strategy gate wired via `MigrationSourceRoot { get; init; }`; runs BEFORE topology capture. **17 scanner unit tests + 2 strategy-gate tests = 19 new tests**; squash suite 593 -> 612. **Cross-provider hoist candidate CONFIRMED** for Phase 5: scanner shape is now identical across FOUR providers (Postgres, Aerospike, OpenSearch, MongoDB); `MigrationSourceScannerBase` should hoist into core lib with the data-op-detection portion as the only per-provider override.
- **3.8 Unit tests per component (R-P8)** ☑ COMPLETE 2026-05-11 (shipped with Tasks 3.1-3.7) -- 162 new unit tests across 8 test classes.
- **3.9 Determinism gate integration test (R-P5)** ☑ COMPLETE 2026-05-11 -- [MongoDBSquashDeterminismTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/MongoDBSquashDeterminismTests.cs) guarded by `#if INTEGRATIONS`. 4 integration tests: empty-database determinism, populated-database determinism (creates 2 collections + unique index, captures twice via full strategy pipeline, asserts byte-equal), collection-creation-order independence, recreate-with-different-UUID determinism (proves uuid stripping works across re-creations).
- **3.10 Verification-round integration test (R-P6)** ☑ COMPLETE 2026-05-11 -- [MongoDBSquashVerificationTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/MongoDBSquashVerificationTests.cs) guarded by `#if INTEGRATIONS`. 2 integration tests: empty-database round-trip baseline, populated-database round-trip with full apply-and-recapture. Apply helper walks the canonical JSON sections and recreates each collection + index via the MongoDB driver.
- **3.11 CLI integration test (R-P9)** -- DEFERRED to Phase 5 (consistent with Aerospike Phase 1 Task 1.11 + OpenSearch Phase 2 Task 2.11).
- **3.12 ADR-0019 amendment check (R-P7)** ☑ COMPLETE 2026-05-11 -- NO CHANGES REQUIRED. Third consecutive provider phase shipping without ADR-0019 amendments. The 5-interface contract absorbs MongoDB cleanly. **Appendix B contract snapshot unchanged for the fifth time** (Phase 0 + Phase 1 + Phase 2 + Phase 3).
- **3.13 Phase 3 boundary** ☑ COMPLETE 2026-05-11 -- ☑ unit suite green (squash 612/612 on net10, +162 new from Phase 3 start); ☑ core suite green (356/356); ☑ integration project builds clean both with `-p:EnableIntegrationTests=true` (0 errors) and without; ☑ plan checkboxes flipped; ☑ zero new bugs surfaced during testing (first phase with no bug discovery -- cross-provider carry-forwards from 2 prior providers absorbed MongoDB without surprise). Ready for Phase 4 Couchbase.
...
### Phase 4: Couchbase squash codegen (R-P3, R-P5-R-P9) (~2 weeks) -- moved from Phase 3 (2026-05-11) ☑ COMPLETE 2026-05-11
...
- **4.1 `CouchbaseTopologySignature`** ☑ COMPLETE 2026-05-11 -- ~330 LOC; captures 9 axes: server major.minor, edition (Community / Enterprise), services (sorted union from `/pools/default` -- `kv`/`n1ql`/`index`/`fts`/`eventing`/`analytics`/`backup`/`cbas`), bucket name, bucket type (`membase`/`ephemeral`/`memcached`), storage backend (`couchstore`/`magma`), replica count, memory quota (MB). Probes via the existing `ICouchbaseRestApiService` (the test-double seam already in place for runner code): `GetClusterDetailsAsync()` for version + edition + services-per-node; `GetBucketDetailsAsync(bucketName)` for bucket type + storage backend + replica count + memory quota. Edition prefers the `isEnterprise` boolean on `/pools/default` (canonical) with a fallback to suffix-stripped version parsing (`7.2.0-5325-enterprise` -> `Enterprise`). Memory quota converts bytes -> MB at signature-time so the axis is operator-readable. Services union deduplicates across multi-node clusters and sorts ordinal so two clusters with the same service mix in different add-order match. `IsCompatibleWith` rules: major-strict, edition-strict (case-insensitive), bucket name + type + storage-backend strict, replica count strict, memory quota strict, services set equality; minor-version tolerated (the "diagnostic-only axis" pattern from MongoDB Phase 3). **32 unit tests cover:** SchemaVersion + ProviderId stability, self-compatibility, per-axis incompatibility (major / edition / bucket name / bucket type / storage backend / replica count / memory quota / services), minor-tolerated, edition case-insensitive, services-order-independent, cross-provider rejection vs MongoDB, Properties bag shape; plus internal parsers (EE/CE cluster shapes, isEnterprise-missing fallback, version-without-suffix, services union with multi-node + dedup, Couchstore + Magma bucket shapes, ephemeral bucket, storageBackend-missing defaults to empty), null/empty guards on `CaptureAsync`. **Cross-provider observation:** CE-vs-EE edition + services matrix played the Edition-axis role exactly as predicted by OpenSearch (plugin matrix) + MongoDB (FCV scalar). Couchbase sits between the two: scalar edition like FCV, sorted set like the plugin matrix. The existing `ITopologySignature` `Properties` bag absorbed both shapes cleanly with no contract change. Strongest cross-provider evidence yet that the contract is correct -- four providers, four different topology axis catalogs, one common signature shape. Test suite: 644/644 squash tests passing (612 + 32).
- **4.2 `CouchbaseDataOpClassifier`** ☑ COMPLETE 2026-05-11 -- ~210 LOC. Dual-shape classifier mirrors Aerospike/OpenSearch/MongoDB precedent. Statement form recognizes a superset of the partial-StatementParser grammar so resource scripts classify without round-tripping: `CREATE [PRIMARY] INDEX|BUCKET|SCOPE|COLLECTION`, `DROP [PRIMARY] INDEX|BUCKET|SCOPE|COLLECTION`, `BUILD INDEX`, `ALTER INDEX` -> structural; `INSERT INTO`, `UPSERT INTO`, `UPDATE`, `DELETE FROM`, `MERGE INTO` -> data op. Call-site form (no receiver-name anchor, same MongoDB-style trade-off documented): KV writes (`Upsert/Insert/Replace/Remove/MutateIn/Append/Prepend/Increment/Decrement/Unlock`) -> data op; KV reads (`Get/GetAnyReplica/GetAllReplicas/GetAndLock/GetAndTouch/Touch/Exists/LookupIn`) -> read; FTS `SearchAsync` -> read; sub-client structural paths (`.QueryIndexes.|.Buckets.|.Collections.|.Scopes.|.SearchIndexes.|.AnalyticsIndexes.|.AnalyticsLinks.|.EventingFunctions.|.Users.|.Groups.|.Views.|.ViewIndexes.`) -> structural. **R-P3 OQ resolution -- parameterized N1QL:** `.QueryAsync` and `.AnalyticsQueryAsync` source-only inspection cannot resolve the SQL value reliably (variables, interpolation, builders, helper methods). Treated as **default-deny** -- falls through to the unclassified branch with `RequiresAnnotation` set. Operators annotate with `[DataMigration]` when the N1QL writes, `[StructuralOnly]` when it reads or runs DDL. False-positive (annotation when not strictly needed) is cheaper than false-negative (data op silently squashed). Statement-form DML regex still recognizes raw DML leading-keywords because resource scripts ship those strings literally -- only call-site form requires annotation. Carries forward the cross-provider patterns: `MethodCallTail` regex (generic-method anchoring -- Couchbase SDK methods are `collection.UpsertAsync<T>`), shared non-determinism catalog (DateTime/DateTimeOffset/Guid/Random/Stopwatch). **54 unit tests cover** statement-form structural (13) + statement-form DML (5) + call-site writes (8, including generic method anchor + Binary sub-property) + call-site reads (5, including FTS) + call-site structural sub-clients (8) + R-P3 default-deny for QueryAsync + AnalyticsQueryAsync (2) + default-deny / empty / null / precedence (4) + non-determinism (9 incl. ScanNonDeterminism helper). 54/54 first run. **Cross-provider observation:** the receiver-anchoring decision matches MongoDB's exactly (Couchbase + MongoDB both route through local vars; OpenSearch + Aerospike both use `_client`). Two receiver-anchoring camps now established; the contract absorbs both without modification.
- **4.3 `CouchbaseStatementClassifier`** ☑ COMPLETE 2026-05-11 -- ~110 LOC. Thin projection over the existing `StatementParser` ([src/Hyperbee.Migrations.Providers.Couchbase/Parsers/StatementParser.cs](../../../src/Hyperbee.Migrations.Providers.Couchbase/Parsers/StatementParser.cs)). Maps 10 `StatementType` enums 1:1 to `CouchbaseStatementKind`: `CreateBucket`, `CreateIndex`, `CreatePrimaryIndex`, `CreateScope`, `CreateCollection`, `DropBucket`, `DropScope`, `DropCollection`, `Update`, `BuildIndex`. Output record: `ClassifiedStatement(Kind, Namespace, BucketName, ScopeName, CollectionName, IndexName, Body, Detail)` -- projects through the four-part KeyspaceRef shape (`[namespace:]bucket[.scope.collection]`) explicitly. Default-deny on parser failure with diagnostic in Detail; the partial parser doesn't consume DML (INSERT/UPSERT/UPDATE/DELETE/MERGE -- the data-op classifier handles those via leading-keyword regex, independent of the parser). 19/19 unit tests first run -- 13 happy-path kinds (including primary-index-with-name, anonymous primary-index, namespaced keyspace, backtick-quoted identifier) + 6 fallback paths (DML / SELECT / garbage / empty / whitespace / null). **Cross-provider observation:** Now FOUR thin-projection classifiers (Aerospike + OpenSearch + MongoDB + Couchbase). The projection pattern (lift the parser's tuple-typed `Then` callback output to a typed `ClassifiedStatement` record with provider-specific identifier slots; default-deny on `NotSupportedException`) is locked. Phase 5 hoist candidate: `ClassifiedStatementBase` core + per-provider derived records carrying the identifier shape. Defer to Phase 5 release prep so we don't risk contract drift mid-flight.
- **4.4 `CouchbaseSnapshotCanonicalizer`** ☑ COMPLETE 2026-05-11 -- ~290 LOC; section-headered blob -> canonical normalized output, **JSON-section form** (same precedent as MongoDB Phase 3 -- Couchbase structural state (bucket settings WITH clauses, N1QL index WHERE/USING GSI options, FTS index definitions, Eventing function source) exceeds the partial-grammar StatementParser, so canonical form ships JSON-section per Task 2.0 opaque-content + structural-canonical split). Sections: `[buckets]` / `[scopes]` / `[collections]` / `[indexes]` / `[fts_indexes]` / `[eventing_functions]` / `[users]` / `[groups]` plus unknown-section pass-through. **Canonicalization steps** (carry-forward from MongoDB/OpenSearch): JSON-parse each section; recursively sort object keys ordinal; strip ephemeral catalog (`id`, `docCount`, `dataSize`, `memUsed`, `diskUsed`, `opsPerSec`, `itemCount`, `lastUsed`, `lastRebalanceTimestamp`/`last_rebalance_timestamp`, `nodes`, `nodeUUID`, `uuid`, `vBucketServerMap`, `quotaUsed`, `quotaPercentUsed`, `basicStats`) at every nesting level; re-emit with `Utf8JsonWriter` (Indented=true + UnsafeRelaxedJsonEscaping); CRLF→LF normalization; sections emit alphabetical. **R-P3 Open Question resolution -- deferred-build index preservation:** the `[indexes]` section has section-aware `state` handling. `state=online` is dropped (default, no structural information); `state=deferred` is **preserved** so the apply path can issue BUILD INDEX after CREATE INDEX; transient values (`building`/`pending`/`scheduled`/`deleted`) throw a `MigrationException` at canonicalization time -- the capture strategy must wait for indexes to settle before snapshotting. The deferred-state rule is scoped to `[indexes]` only; a `state` field anywhere else (e.g., a bucket's runtime state) passes through unmodified. **Opaque-content preservation** per Task 2.0 spike + MongoDB Phase 3 precedent: Eventing JS source, N1QL WHERE clauses, FTS index definitions ride through as JSON value content -- the canonicalizer never parses N1QL, JS, or FTS semantically. Numbers preserve operator representation via `GetRawText`. Arrays preserve declared order (GSI compound-index `index_key` arrays are order-sensitive). **EmitScript = Canonicalize (identity)** for v3.0 -- the JSON-section form IS the embeddable form; richer statement-form emission is a Phase 5 polish item if needed. **31 unit tests cover:** section parsing + ordering + case-insensitive headers, recursive key sort (top-level + nested), array order preservation, ephemeral stripping at every level + bucket runtime stats + index id + last_rebalance_timestamp + nodes + vBucketServerMap, deferred-state preservation (online dropped, deferred preserved, building/pending throws, non-[indexes] state passes through), opaque content preservation (eventing JS + N1QL WHERE), numbers preserved, idempotence (C12 baseline), divergent key orders / divergent ephemerals producing byte-equal output, line-ending normalization, invalid-JSON error handling, unknown-section pass-through, null-throws, EmitScript identity. 31/31 first run after one wrong-test-assumption fix (mid-section comments are part of the body per the MongoDB-inherited parser shape; clarified by moving the comment test to leading-comments-only). **Cross-provider observation:** the JSON-section + ephemeral-strip + opaque-content split is now the cross-provider canonical pattern -- 3 providers (OpenSearch, MongoDB, Couchbase) ship it with the same `Utf8JsonWriter` + ordinal-sort scaffold; only the ephemeral catalog + section names differ. Phase 5 hoist candidate: `JsonSectionCanonicalizerBase` carrying the parse/sort/strip/emit/normalize scaffolding; per-provider derived class carries the ephemeral catalog + section-aware hooks (the `state` deferred-build rule).
- **4.5 `HybridStrategy : ISquashStrategy` + `CouchbaseSquashGenerationContext` + `CouchbaseSnapshotCapture`** ☑ COMPLETE 2026-05-11 -- ~190 + ~95 + ~200 LOC. `HybridStrategy` orchestrates the V1 happy path (validate -> source-scan gate -> topology capture -> bounds -> snapshot capture -> canonicalize -> emit) -- structurally identical to `IntrospectionSnapshotStrategy` from Phase 3 MongoDB. `CouchbaseSquashGenerationContext` carries the live `ICluster` (for N1QL system table queries) + `ICouchbaseRestApiService` (for bucket/scope settings the system tables omit) + `BucketName` + `CaptureSnapshotAsync` delegate -- this is the "hybrid" nature: two capture sources combined. `CouchbaseSnapshotCapture.CaptureAsync` probes via `SELECT keyspaces.* FROM system:keyspaces WHERE bucket = $bucket` + `SELECT indexes.* FROM system:indexes WHERE keyspace_id = $bucket OR bucket_id = $bucket` + REST `/pools/default/buckets/<name>`; `ComposeBlob` assembles section-headered output keyed by stable identity tuples (`keyspace_id` for keyspaces, `bucket/scope/keyspace/name` for indexes). 19 unit tests cover ComposeBlob (5: section headers, ordering, all-empty header-only, canonicalizer round-trip with ephemeral strip + deferred-state preservation, empty-bucket throws) + CaptureAsync guard rails (3: null cluster / null restApi / empty bucket name) + Strategy guard rails (5: null context / empty descriptors / wrong context type / providerId / null deps / null logger) + Context validation (2: all-fields required + ProviderId) + Source-scan refusal gate (2: unannotated refused / all-annotated proceeds-past-gate). **DI wiring complete this task**: `AddCouchbaseMigrations` now registers all 5 squash components plus the `SquashStrategyDescriptor` whose ctor `EnsureValid` asserts ProviderId agreement across all 5 component instances.
- **4.6 `CouchbaseSquashVerifier`** ☑ COMPLETE 2026-05-11 -- ~150 LOC; A4 byte-equality round. Structurally near-copy-paste from Aerospike Task 1.6 + OpenSearch Task 2.6 + MongoDB Task 3.6 -- same ctor (canonicalizer), same `CaptureFromGeneratedAsync` init-property, same `VerifyAsync` flow, same SummarizeDiff truncation. **19 unit tests cover** verifier (12: providerId wiring, ctor null-guard, wrong-context-type returns Failed, missing capture delegate returns Failed, null Generated returns Failed, empty Replaces returns Failed, matching snapshots return Success, divergent-snapshot Failed with diff naming both index names, ephemeral-only-difference Success defense-in-depth -- proves `id` + `docCount` stripping makes A/B byte-equal, exception cause capture, cancellation propagates, SummarizeDiff truncation) + DI wiring (7: each component resolves, descriptor passes EnsureValid, singleton semantics, descriptor components are same instances as direct resolutions). 19/19 first run. **Cross-provider observation:** verifier shape is now byte-for-byte structurally identical across FOUR providers (Aerospike, OpenSearch, MongoDB, Couchbase). The `ISquashVerifier` contract is rock-solid -- four providers, four near-identical implementations differing only in the topology type and the section names in test fixtures. Phase 5 hoist candidate confirmed: `SquashVerifierBase` carrying the entire VerifyAsync flow + SummarizeDiff; per-provider derived class only carries the typed `CaptureFromGeneratedAsync` signature.
- **4.7 `CouchbaseMigrationSourceScanner`** ☑ COMPLETE 2026-05-11 -- ~210 LOC; Roslyn-based source scanner. Mirrors `MongoDBMigrationSourceScanner` shape with Couchbase-specific KV write verb sets (`Upsert`/`Insert`/`Replace`/`Remove`/`MutateIn`/`Append`/`Prepend`/`Increment`/`Decrement`/`Unlock` + Async variants; 20 method-name variants). No receiver-name filter (Couchbase code routes through local `collection`/`bucket`/`scope`/`cluster` vars -- MongoDB-camp on the receiver-anchoring axis). **R-P3 OQ acknowledgement in scanner:** `QueryAsync` and `AnalyticsQueryAsync` deliberately omitted from the data-op verb catalog -- source-only inspection cannot resolve the SQL value; the data-op classifier surfaces default-deny at classification time, scanner agrees. Non-determinism scan same catalog as Aerospike. Wired into `HybridStrategy.MigrationSourceRoot` gate (runs BEFORE topology capture so a refused squash does not waste cluster probes). **18 scanner tests cover** structural-only / KV write families / generic Upsert / read-only no-match / R-P3 OQ QueryAsync-not-flagged / non-determinism (DateTime/Guid/Random/seeded-Random) / class-not-extending-Migration / no-Migration-attribute / Attribute-suffix-form / DirectoryNotFound / empty-sourceRoot / null source. **Cross-provider hoist candidate confirmed:** the Migration-extends + attribute recognizer + non-determinism scan + ClassVerdict.RequiresAnnotation shape is now identical across FIVE providers (Postgres, Aerospike, OpenSearch, MongoDB, Couchbase). Phase 5 release prep should hoist `MigrationSourceScannerBase` into the core library so per-provider scanners only override the data-op-method catalog + (when relevant) the receiver-anchor predicate.
- **4.8 Unit tests** ☑ COMPLETE 2026-05-11 -- 192 total Couchbase squash tests, all passing first run (after one wrong-test-assumption fix in canonicalizer comments handling, and one wrong-test-assertion fix in ComposeBlob round-trip where `def` from a substring assertion matched `_default` in the keyspace identity tuple). Cumulative squash test count: 803/803 (612 + 192 Couchbase). Core suite 356/356. Integration project builds clean both with `-p:EnableIntegrationTests=true` (0 errors) and without (only pre-existing OpenSearch MSTEST0037 warnings).
- **4.9 Determinism gate integration test (R-P5)** ☑ COMPLETE 2026-05-11 -- [CouchbaseSquashDeterminismTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/CouchbaseSquashDeterminismTests.cs). Three `#if INTEGRATIONS`-guarded tests: `EmptyBucket_TwoRuns_ProduceByteEqualContent` (canonical no-state case), `PopulatedBucket_TwoRuns_ProduceByteEqualContent` (primary + secondary GSI indexes; defense-in-depth + byte-equal), `IndexRecreatedWithDifferentId_ProducesByteEqualContent` (ephemeral-strip proof -- server reassigns `id` on recreate, canonicalizer strips). The test class connects to the existing `CouchbaseTestContainer`'s `hyperbee` bucket via `couchbase://localhost` (the ports CouchbaseTestContainer binds to host), constructs a host-side `ICluster` + `CouchbaseRestApiService`, exercises the full `HybridStrategy.GenerateAsync` path with live capture (system:keyspaces + system:indexes + REST `/pools/default/buckets/hyperbee`). InternalsVisibleTo for `Hyperbee.Migrations.Integration.Tests` added to the Couchbase provider's csproj so the test can construct `CouchbaseRestApiService` directly (internal class). Integration project builds clean both with `-p:EnableIntegrationTests=true` (0 errors) and without (default).
- **4.10 Verification round integration test (R-P6)** ☑ COMPLETE 2026-05-11 -- [CouchbaseSquashVerificationTests.cs](../../../tests/Hyperbee.Migrations.Integration.Tests/CouchbaseSquashVerificationTests.cs). Two `#if INTEGRATIONS`-guarded tests: `EmptyBucket_RoundTrip_ReturnsSuccess` and `PopulatedBucket_RoundTrip_ReturnsSuccess`. Round-trip flow: capture A (historical) -> generate -> drop our indexes -> apply generated content (walks the canonical `[indexes]` section + recreates via SDK `QueryIndexes.CreatePrimaryIndexAsync`/`CreateIndexAsync`) -> capture B -> verify byte-equal. Minimum-viable apply path for v3.0 -- the full Phase 5 CLI ships bucket/scope/collection/eventing/FTS apply too. Mirrors MongoDB Phase 3 Task 3.10 + OpenSearch Phase 2 Task 2.10 shape: A4 byte-equality round through a fresh apply, structural state recovered without any data side effects.
- **4.11 CLI integration test (R-P9)** -- deferred to Phase 5 release prep (consistent with the per-phase deferral for CLI tests across all four providers).
- **4.12 ADR-0019 amendment check** -- none surfaced through 4.1-4.10 (FOUR providers shipped without amendment; the contract is rock-solid).
- **4.13 Phase 4 boundary** ☑ COMPLETE 2026-05-11 -- ☑ unit suite green (squash 803/803 on net10, +192 new from Phase 4 start: 32 topology + 54 data-op + 19 statement + 31 canonicalizer + 19 verifier + 7 DI wiring + 18 scanner + 16 end-to-end); ☑ core suite green (356/356); ☑ integration project builds clean both with `-p:EnableIntegrationTests=true` (0 errors) and without; ☑ plan checkboxes flipped; ☑ NO ADR-0019 amendments required (FOUR consecutive phases without amendment). ALL FOUR NON-POSTGRES PROVIDERS SHIPPED.
...
- Phase 0: ☑ COMPLETE 2026-05-11
- Phase 1 (Aerospike): ☑ COMPLETE 2026-05-11
...
- *Phase 2:* ☑ COMPLETE 2026-05-11 — OpenSearch squash codegen (the contract-pressure-test pass) shipped: 8 components (~1660 LOC across `src/Hyperbee.Migrations.Providers.OpenSearch/Squash/`) including the Roslyn source scanner; 186 new unit tests (squash suite 264 → 450, +186); 5 integration tests (3 R-P5 determinism + 2 R-P6 verification, all `#if INTEGRATIONS`-guarded). Painless spike (Task 2.0) concluded "opaque-string preservation + structural JSON canonicalization" -- no painless parser, no operator annotation, no fallback needed. Contract validated: NO ADR-0019 amendments required despite OpenSearch being the highest-risk provider (multi-axis plugin matrix, JSON-bodied canonical form, opaque painless content). Pre-existing parser bug surfaced + fixed (`MIGRATE INDEX ... WITH BODY @path` mis-cast). Cross-provider carry-forwards confirmed for MongoDB Phase 3 + Couchbase Phase 4: (1) MigrationSourceScannerBase hoist confirmed for Phase 5 (now 3 providers using identical scanner shape); (2) MethodCallTail regex pattern (generic-method anchoring) applies to MongoDB Builders<T> + Couchbase UpsertAsync<T>; (3) Utf8JsonWriter CRLF→LF normalization applies preemptively; (4) verifier shape is byte-for-byte structurally identical across two providers; (5) opaque-content + structural-canonical split applies to MongoDB BSON aggregation pipelines + Couchbase N1QL UDF source. Ready for Phase 3 MongoDB.
- *Phase 3:* ☑ COMPLETE 2026-05-11 — MongoDB squash codegen shipped without surprise. 8 components (~1510 LOC across `src/Hyperbee.Migrations.Providers.MongoDB/Squash/`) + 162 new unit tests + 6 integration tests (4 R-P5 determinism + 2 R-P6 verification, all `#if INTEGRATIONS`-guarded). **Zero new bugs surfaced during testing** -- first phase with no bug discovery. The cross-provider carry-forwards from Aerospike + OpenSearch (MethodCallTail regex, Utf8JsonWriter LF normalization, opaque-content + structural-canonical split, ILogger injection, MigrationSourceRoot init-property, 5-interface contract) all applied successfully WITHOUT bug discovery. Contract validated: NO ADR-0019 amendments required (third consecutive phase). FCV played the Edition-axis role exactly as predicted -- single scalar, fit cleanly. Sharded-takes-precedence-over-replica-set deployment classification edge case (mongos response includes BOTH msg=isdbgrid AND setName for the config replica set) handled cleanly. Verifier shape is now byte-for-byte structurally identical across THREE providers; MigrationSourceScannerBase hoist candidate confirmed across FOUR providers. **Plan rationale revision:** original Task 3.4 plan called for statement-form canonicalizer emission; actual implementation ships JSON-section form because MongoDB's structural state (validators, time-series options, view pipelines, partialFilterExpression on indexes) exceeds the narrow MongoStatementParser grammar. Documented inline. Ready for Phase 4 Couchbase.
- *Phase 4:* ☑ COMPLETE 2026-05-11 — Couchbase squash codegen shipped on the same cadence as MongoDB Phase 3. **6 components (~1480 LOC across `src/Hyperbee.Migrations.Providers.Couchbase/Squash/`) + 192 new unit tests + 5 integration tests (3 R-P5 determinism + 2 R-P6 verification, all `#if INTEGRATIONS`-guarded). Cumulative squash test count: 803/803 (612 baseline + 192 Couchbase). **TWO new bugs surfaced + fixed at test-time**, both in the test suite itself (mid-section comments are part of the body per the parser shape inherited from MongoDB; substring assertion `.NotContain("def")` matched `_default` in the keyspace identity tuple); zero bugs in production code. Contract validated: **NO ADR-0019 amendments required for FOUR consecutive phases** -- the strategy contract is rock-solid. **R-P3 OQ resolutions**: (a) parameterized N1QL `QueryAsync` / `AnalyticsQueryAsync` source-only inspection cannot resolve the SQL value reliably, so the data-op classifier + Roslyn source scanner BOTH default-deny these call-sites -- operators annotate with `[DataMigration]` / `[StructuralOnly]` explicitly; (b) deferred-build GSI indexes preserved at canonicalization time -- `state=online` dropped (default, no structural information), `state=deferred` preserved (apply path issues BUILD INDEX), transient values (`building`/`pending`/`scheduled`/`deleted`) throw loudly so the squash CLI's settle-wait bug surfaces immediately. **CE-vs-EE Edition axis** played the predicted role -- single scalar like MongoDB's FCV, sorted set like OpenSearch's plugin matrix, the existing Properties bag absorbed both shapes without contract change. **Cross-provider hoist confirmation:** with FIVE providers shipped (Postgres baseline + Aerospike + OpenSearch + MongoDB + Couchbase), Phase 5 hoist candidates are now (1) `MigrationSourceScannerBase` (five providers using identical scanner shape), (2) `JsonSectionCanonicalizerBase` (three providers using identical canonicalizer scaffold), (3) `SquashVerifierBase` (four providers using byte-for-byte identical verifier), (4) `ClassifiedStatementBase` (four providers using identical projection shape). Plus integration-test wiring lessons: InternalsVisibleTo for Integration.Tests added to Couchbase provider so the test can construct `CouchbaseRestApiService` directly; host-side `ICluster` + `CouchbaseRestApiService` pattern lifts cleanly into Phase 5 CLI. **ALL FOUR NON-POSTGRES PROVIDERS SHIPPED** -- the v3.0 release-blocking work (per R-P1/R-P2/R-P3/R-P4 + the all-providers-or-nothing rule) is functionally done; Phase 5 release prep is the remaining work.
- *Phase 5:* [pending]
