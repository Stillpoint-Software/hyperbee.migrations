...
| 2026-05-16 | negative->fixed | Phase 5 final CI (25977279302, docs-only on 888238b) surfaced a deeper root cause the Phase-3 fix did NOT cover: net9-couchbase failed 3 squash tests (`IndexRecreatedWithDifferentId`, `GenerateAsync_AgainstSampleAssembly`, `PopulatedBucket_RoundTrip`), each exhausting the 180s `WithRebalanceRetryAsync` backstop on the test's **FIRST** CREATE INDEX. The exact same code passed 23/23 on 65b7a14 (25976819073) ~25 min earlier -> flake, not regression -> root-cause per the standing rule, not a retrigger. Root cause: `ClusterIdleAsync` gates only the cluster KV `rebalance` task (`/pools/default/tasks` type==rebalance); the GSI **index service**'s own initial topology placement on a fresh single-node cluster is not reported there, so `WaitUntilClusterIdleAsync` returns while the indexer still rejects DDL with "rebalance in progress". On a fast box the indexer settles before the test's first CREATE; on a CPU-throttled CI runner it outlasts the 180s backstop. Phase 3's `WaitForIndexReadyAsync` serializes create-AFTER-create -- it cannot help the FIRST create (nothing prior to wait on). Fix: `IsolatedCouchbaseContainer.WarmupIndexerDdlAsync` -- a sentinel CREATE PRIMARY/Ready/DROP/Gone with a generous 5-min bound, run after `WarmupN1qlAsync`, before any test body. Proves the indexer accepts DDL so real creates never race a cold indexer. Fixture-layer only: zero runtime / squash-output / grammar change (consistent with the Phase 3 false-analogy lesson + ADR constraints); mirrors the existing `WarmupN1qlAsync` planner-warmup precedent. Validated locally: all 6 IsolatedCouchbaseContainer Couchbase squash tests green (Determinism+Verification 5/5 4m47s; ProviderIntegration 1/1 2m25s). Lesson: "wait for cluster idle" is necessary but NOT sufficient for GSI DDL -- the index service has a separate readiness axis the cluster-tasks endpoint does not expose; gate the actual gated operation (a sentinel DDL), not a proxy signal. |
