...
// find ISquashProvider implementations. These tests pin the discovery
...
[TestClass]
public class SquashProviderRegistryTests
{
    [TestMethod]
    public void Discover_FindsTestProviderInThisAssembly()
    {
        var providers = SquashProviderRegistry.Discover( typeof( SquashProviderRegistryTests ).Assembly );
    ...
    {
        Action act = () => SquashProviderRegistry.Discover( null! );
...
public sealed class TestCliProvider : ISquashProvider
{
    ...
    public Task<SquashGenerationResult> GenerateAsync(
        SquashRequest context, CancellationToken cancellationToken )
...
public abstract class AbstractCliProvider : ISquashProvider
{
    public string ProviderId => "abstract";
    public string SquashFileExtension => ".test";

    public Task<SquashGenerationResult> GenerateAsync( SquashRequest context, CancellationToken ct )
        => throw new NotSupportedException();
...
internal sealed class InternalCliProvider : ISquashProvider
{
...
public sealed class NoDefaultCtorCliProvider : ISquashProvider
{
